...
void heigh_cal (TFile *frun, int run, int ch=0, int Height_nbins=300, double* phe_height=0, double* phe_sigma=0);
...
void calibration_analysis (int run, int Height_nbins=300, int area_nbins=350)
{
    ...
    if(run<10) frun = new TFile(Form("../../../analyzed/run0000%d.mid.lz4.root",run),"READ");
    if(run>=10 && run<100) frun = new TFile(Form("../../../analyzed/run000%d.mid.lz4.root",run),"READ");
    if(run>=100) frun = new TFile(Form("../../../analyzed/run00%d.mid.lz4.root",run),"READ");
    ...
    //
    //Calibración y ajuste en altura
    //
    ...
    { 
      ...
      heigh_cal (frun, run, ch, Height_nbins, &phe_height[ch], &phe_sigma[ch]);
    }  
    
    c3->SaveAs(Form("./Plots/Calibration_analysis/Height_channels_fit_%d.png",run));
...
void phe_calibration (TFile *frun, int run, int ch=0, double* cal=0, double* cal_err=0, double* res=0, double* res_err=0, int Height_nbins=300, int area_nbins=350)
{
  ...
  if(run==734)
  {
    ...
    /*for(k=0; k<3; k++)
    {
    	line = new TLine(Height_cut[k],0,Height_cut[k],3000); //Coordenadas de la línea
    	line->SetLineWidth(4.); //Tamaño de la línea
      line->SetLineStyle(9);
    	line->SetLineColor(kBlack); //Color de la línea
    	line->Draw("same"); //Para dibujar la línea
    }*/
...
void heigh_cal (TFile *frun, int run, int ch=0, int Height_nbins=300, double* phe_height, double* phe_sigma)
{   
    TTree * pulses = (TTree*)frun->Get("ta");
    ...
    *phe_sigma = gaus[1]->GetParameter(2);
    ...
    Height->GetXaxis()->SetTitle("Height (ADC)");
...
void fwhm_distribution (TFile *frun, int run, int ch=0, int Height_nbins=300, double phe_height=0, double phe_sigma=0, TProfile* one_phe=0)
{
  ...
  TPaveText *pv = new TPaveText(0.50,0.75,0.83,0.85,"NDC"); //Coordenadas del texto (x0,y0,x1,y1)
    pv->SetBorderSize(0); //Tamaño del borde
    pv->SetFillColor(0); //Color del fondo
    pv->SetTextSize(.05); //Tamaño del texto
    ...
    pv->SetTextAlign(22); //Alineación del texto 22=centrada en horizontal y vertical  
