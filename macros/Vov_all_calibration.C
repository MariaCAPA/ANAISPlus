...
void Vov_all_calibration (int run_i=532, int run_f=545, const char* dir="%s", double Vov_i=4, double delta_Vov=0.5, double Vov_err=0.55)
{
  ...
  {
    std::ifstream inputfile(Form("./Plots/Calibration_analysis/Channel_calibration_%d.txt",run), std::ios::in );
  ...
  //Calibramos
  //
  TF1* cal0 = new TF1("cal0","x*[0]",0,11);
  TF1* cal1 = new TF1("cal1","x*[0]",0,11);
  TF1* cal2 = new TF1("cal2","x*[0]",0,11);
  TF1* cal3 = new TF1("cal3","x*[0]",0,11);
  TF1* cal4 = new TF1("cal4","x*[0]",0,11);
  TF1* cal5 = new TF1("cal5","x*[0]",0,11);
  TF1* cal6 = new TF1("cal6","x*[0]",0,11);
  TF1* cal7 = new TF1("cal7","x*[0]",0,11);
  
  FILE *f;
  f=fopen(Form("./Plots/%s/Channel_Vov_cal_%d_%d.txt",dir,run_i,run_f),"w");
  ...
  cal0->SetTitle("Calibration");
  cal0->GetYaxis()->SetTitle("Phe gain (ADC)");
  cal0->GetXaxis()->SetTitle("Vov (V)");
  
  cal0->GetYaxis()->SetRangeUser(0,2600);
  ...
  c1->SaveAs(Form("./Plots/%s/Vov_calibration_%d_%d.png",dir,run_i,run_f));
  ...
  cal_ch0->GetYaxis()->SetRangeUser(600,2600);
  cal_ch1->GetYaxis()->SetRangeUser(600,2600);
  cal_ch2->GetYaxis()->SetRangeUser(600,2600);
  cal_ch3->GetYaxis()->SetRangeUser(600,2600);
  cal_ch4->GetYaxis()->SetRangeUser(600,2600);
  cal_ch5->GetYaxis()->SetRangeUser(600,2600);
  cal_ch6->GetYaxis()->SetRangeUser(600,2600);
  cal_ch7->GetYaxis()->SetRangeUser(600,2600);
  ...
  for(ch=0; ch<=7; ch++)
    { 
      ...
      info = new TPaveText(0.15,0.73,0.53,0.88,"brNDC");
      info->SetTextSize(0.05); //Tamaño del texto
      info->SetFillColor(kWhite); //Fondo de la leyenda
      info->SetBorderSize(0);
  
      info2 = new TPaveText(0.15,0.73,0.53,0.88,"brNDC");
      ...
      if(ch==0)
      {
        info = new TPaveText(0.25,0.73,0.63,0.88,"brNDC");
        info->SetTextSize(0.05); //Tamaño del texto
        info->SetFillColor(kWhite); //Fondo de la leyenda
        info->SetBorderSize(0);
      }
      ...
      if(ch==4)
      {
        info2 = new TPaveText(0.25,0.73,0.63,0.88,"brNDC");
        info2->SetTextSize(0.05); //Tamaño del texto
        info2->SetFillColor(kWhite); //Fondo de la leyenda
        info2->SetBorderSize(0);
      }
      ...
      if(ch==0) {info->AddText(Form("Cal = %.1f #pm %.1f",cal0->GetParameter(0),cal0->GetParError(0))); info->AddText(Form("#chi^{2}/ndf = %.2f",cal0->GetChisquare()/cal0->GetNDF()));}
      if(ch==1) {info->AddText(Form("Cal = %.1f #pm %.1f",cal1->GetParameter(0),cal1->GetParError(0))); info->AddText(Form("#chi^{2}/ndf = %.2f",cal1->GetChisquare()/cal1->GetNDF()));}
      if(ch==2) {info->AddText(Form("Cal = %.1f #pm %.1f",cal2->GetParameter(0),cal2->GetParError(0))); info->AddText(Form("#chi^{2}/ndf = %.2f",cal2->GetChisquare()/cal2->GetNDF()));}
      if(ch==3) {info->AddText(Form("Cal = %.1f #pm %.1f",cal3->GetParameter(0),cal3->GetParError(0))); info->AddText(Form("#chi^{2}/ndf = %.2f",cal3->GetChisquare()/cal3->GetNDF()));}
      
      if(ch==4) {info2->AddText(Form("Cal = %.1f #pm %.1f",cal4->GetParameter(0),cal4->GetParError(0))); info2->AddText(Form("#chi^{2}/ndf = %.2f",cal4->GetChisquare()/cal4->GetNDF()));}
      if(ch==5) {info2->AddText(Form("Cal = %.1f #pm %.1f",cal5->GetParameter(0),cal5->GetParError(0))); info2->AddText(Form("#chi^{2}/ndf = %.2f",cal5->GetChisquare()/cal5->GetNDF()));}
      if(ch==6) {info2->AddText(Form("Cal = %.1f #pm %.1f",cal6->GetParameter(0),cal6->GetParError(0))); info2->AddText(Form("#chi^{2}/ndf = %.2f",cal6->GetChisquare()/cal6->GetNDF()));}
      if(ch==7) {info2->AddText(Form("Cal = %.1f #pm %.1f",cal7->GetParameter(0),cal7->GetParError(0))); info2->AddText(Form("#chi^{2}/ndf = %.2f",cal7->GetChisquare()/cal7->GetNDF()));}
      
      if(ch<=3) info->Draw("same");
      if(ch>3) info2->Draw("same");
    } 
  
  c2->SaveAs(Form("./Plots/%s/Vov_calibration_individual_channels_%d_%d.png",dir,run_i,run_f));
}
